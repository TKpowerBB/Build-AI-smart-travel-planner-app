  ...
  return (
    <div className={`bg-white rounded-2xl p-4 shadow-sm border-l-4 ${SUBTYPE_COLORS[card.subtype]}`}>
      <div className="flex items-start gap-3">
        <span className="text-2xl mt-0.5">{card.emoji || SUBTYPE_ICONS[card.subtype]}</span>
        <div className="flex-1 min-w-0">
          ...
          {card.aiNote && (
            <p className="text-xs text-indigo-600 mt-1.5 leading-snug">💡 {card.aiNote}</p>
          )}
          {card.userNote && (
